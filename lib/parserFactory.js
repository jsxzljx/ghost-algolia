    ...
    parse: (post, index) => {
      ...
      if(astChildren.length !== 0) {
        ...
        if(astChildren[0].type !== 'Header') {
        	fragment.feature_image = post.attributes.bg_image;
        	fragment.author = post.clientData.authors[0].name;
        	fragment.author_image = post.clientData.authors[0].profile_image;
